...
- `断点续传`：进行中的分片写成 `<名字>.part`，完整下完才改名；重跑同一个任务时会复用已下好的分片，
  半截分片用 `Range` 接着下（默认开启）。关掉就是每次全新下载、不覆盖已有文件。
- `请求头和 Cookie`：可选请求信息；请求头每行一个，Cookie 使用 `key=value; key2=value2` 格式。

## 断点续传

分片下载（HLS / 片段列表 / JPEG 序列 / 网页嗅探）和直链下载都支持中断后接着下：

1. 下载中的内容写在 `<正式名字>.part`，**只有完整下完才原子改名**成正式名字——所以目录里看到正式名字，
   就说明这个文件是完整的，重跑时直接复用、不再请求正文。
2. 有 `.part`（或大小可疑的同名文件）时，先用 `Range: bytes=<已有长度>-` 问一次服务端：
   `416` = 已经下满（复用）；`206` = 本地那份是半截（接着追加）；`200` = 对端不支持 Range（丢掉本地那份从头下）。
3. 取消或网络中断后 `.part` 会保留，下次点「开始」就从断点继续；什么都没下到的空壳会被清掉。

想强制重新下载：把 `断点续传` 关掉（会落到新名字上，不覆盖旧文件），或者手动删掉 `<保存目录>/_segments`
和残留的 `*.part`。磁力链接的断点由 aria2 的 `.aria2` 控制文件负责（同一个保存目录里会接着下）。
