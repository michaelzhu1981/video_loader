...
- `合并已下载片段`：勾选时用 ffmpeg 把片段合并成单个视频，**合并成功后自动删掉片段文件和合并用的 `.concat.txt` 清单**；
  关闭时（仅「片段列表」模式）片段本身就是产物，会保留在 `<保存目录>/_segments`。
...
4. **合并成功后片段会被删掉**：视频已经是完整一份文件了，`_segments` 里的分片（JPEG 序列模式则是
   那些帧）就没有用了，程序会自动删除，空掉的 `_segments` 目录也一并收走。
   只有这次没合并成时才会留着——取消、ffmpeg 报错、或者合并产物是空文件——正是下次续传要用的那份。

想强制重新下载：直接再点一次「开始」（合并成功后片段已清掉，会整份重下、另存为 `video-1.mp4` 这样的新名字），
或者把「断点续传」关掉。磁力链接的断点由 aria2 的 `.aria2` 控制文件负责（同一个保存目录里会接着下）。
